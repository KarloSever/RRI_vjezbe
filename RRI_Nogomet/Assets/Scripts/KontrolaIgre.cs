...
public class KontrolaIgre : MonoBehaviour 
{
		...
		void Update()
		{
			...
			if (Input.GetMouseButtonUp(0))
			{
				...
				{
					//ubačaj lopte
					Vector3 hitPos = new Vector3(Input.mousePosition.x, Input.mousePosition.y,
						zDubina);
					hitPos = Camera.main.ScreenToWorldPoint(hitPos);
					loptaInstance.transform.LookAt(hitPos);
					loptaInstance.GetComponent<Rigidbody>().AddRelativeForce(Vector3.forward *
						brzinaLopte, ForceMode.Impulse);
		...
		void CreateLopta()
		{
			loptaInstance = Instantiate (loptaPrefab, loptaPrefab.transform.position,
